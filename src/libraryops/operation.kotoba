(ns libraryops.operation
  "OperationActor -- one library/archive operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (LibraryOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Library Governor (:govern) and
  ...
  One graph run = one library/archive operation (intake -> advise ->
  ...
  decision to a human operator (librarian/archivist/conservator). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:item/lend`/`:item/preserve` ALWAYS reach this node
  when the governor is clean -- see `libraryops.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [libraryops.libraryopsllm :as libraryopsllm]
            [libraryops.governor :as governor]
...
(defn build
  ...
  `libraryops.store/Store`).
  opts:
    :advisor      -- a `libraryops.libraryopsllm/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (libraryopsllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LibraryOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (libraryopsllm/-advise advisor store request)]
            {:proposal p :audit [(libraryopsllm/trace request p)]})))

      ;; Library Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
